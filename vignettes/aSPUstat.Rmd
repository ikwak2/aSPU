...
```
`LIPA` have `r length(newP)` SNPs mapped and we can see that there are
many significant SNPs. It makes sense that SPUM(1) have less p-value
then SPUM(inf) since there are multiple significant SNPs. GATES have
...
out.a <- aSPUMpath(kegg9$nP, corrSNP = kegg9$ldmatrix, pow=c(1,2,4,8, Inf),
                   pow2 = c(1,2,4,8),
                   snp.info=kegg9$snp.info, gene.info = kegg9$gene.info,
                   n.perm=1000, Ps = TRUE)
...
when there are many significant genes with similar effects. It makes
sense to have p-value of `r out.h` for HYST.

On the other hand, aSPUMpath adaptively consider all SPUMpath(i,j)
methods.
