...
##
## It gives the p-values of the SPU test and aSPU test based on based on the permutation of residuals.  (This is version 2, matrix version is faster but if it doesn't work, we should use version 1, vector version)
##
...
##
## data(exdat)
## out <- aSPUperm2(exdat$Y, exdat$X, cov = NULL,
...
aSPUpermC2 <- function(Y, X, cov = NULL, model=c("gaussian","binomial"), pow=c(1:8, Inf), n.perm=1000){
...
#        sigma0 = sqrt(sum(yresids^2)/(n-1))
 ...
 #       sigma0<-sqrt(fit1res1$dispersion)
    ...
    T0sC <- calcT0( t(XUs), as.matrix(yresids), as.matrix(pow), n.perm)
